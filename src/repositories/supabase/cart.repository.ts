...
export class SupabaseCartRepository implements ICartRepository {
  ...
  async createCart(data: { userId?: string; guestToken?: string }): Promise<Cart> {
    const { data: newCart, error } = await this.getClient()
      .from('carts')
      .insert([
        {
  ...
  async addItem(cartId: string, item: Omit<CartItem, "id" | "cart_id" | "created_at" | "updated_at">): Promise<CartItem> {
    ...
    await this.getClient().from('carts').update({ updated_at: new Date().toISOString() }).eq('id', cartId);
  ...
  async updateItemQuantity(itemId: string, quantity: number): Promise<CartItem> {
    ...
    await this.getClient().from('carts').update({ updated_at: new Date().toISOString() }).eq('id', data.cart_id);
  ...
  async removeItem(itemId: string): Promise<void> {
    ...
    const { data } = await this.getClient().from('cart_items').select('cart_id').eq('id', itemId).single();
